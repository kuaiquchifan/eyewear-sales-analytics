...
* 主要内容：
  ...
  * BERT文本情感分析的结果解释：LIME、Integrated Gradients + transformers_interpret/Captum
...
* 具体对应的文件：
  * `6-1-data-extract-from-raw-data1.ipynb`: 从来自hugging face的原始数据中提取不同眼镜商品的内容。
  * `6-2-data-format-convert-from-raw-data.ipynb`: 把来自hugging face的原始数据的Amazon JSONL / CSV 文件转成 Parquet 格式
  * `6-3-data-merge-from-customer-complaints.ipynb`: 合并顾客投诉数据。
  * `6-4-customer-review-data-preprocess-for-NLP-BERT-v2.ipynb`: 对评论文本做 BERT NLP情绪分析、BERT结果解释
  ...
  * `6-8-customer-complaint-data-preprocess-for-NLP-BERT-v4.ipynb`:对投诉文本做 BERT NLP情绪分析、BERT结果解释。
...
* 内容要点：
  ...
  * 测试 Gradio GUI 脚本（query → retrieve → generate）。
* 产出与脚本（项目内对应文件）：
* 知识构建：
...
* 检索内容：`7-3-retrieval\retrieval-v2-gui-LangChain.py`：该脚本使用 Python、LangChain、faiss、torch、sentence-transformers、gradio 和 OpenAI 客户端（用于 DeepSeek API）实现一个基于向量检索的 RAG 演示：它载入已构建的 FAISS 索引与 knowledge_meta.json，用本地 SentenceTransformer 模型将用户查询编码为向量并在索引中检索相似片段（只保留来源包含 customer_cluster 的条目），把检索到的上下文拼成特定格式的 prompt 发给 DeepSeek（通过 OpenAI API 客户端），然后在一个 Gradio GUI 中展示 LLM 的回答和检索结果。
* 演示成果： `final_output/` 下的示例截图。

### 8-promo-effect

* 目的：评估促销活动对销售表现的增量影响与归因。
* 分析方法：
  * 前后对比、同店同商品基线对照、活动窗口与等长基线窗口比较。
  * 通过销售额、销量、毛利、AOV、毛利率与 ROI 等指标衡量活动效果。
  * 计算活动增量收益与基线对比收益，并输出效果评估表。
* 具体对应的文件：`8-1_promo_effect.ipynb.py`
* 输出：促销效果评估结果表、Parquet 输出文件、可复现的 notebook。

### 9-review-complaint-overview

* 目的：汇总客户评论与投诉数据，并为后续的主题分析、投诉率分析与趋势分析提供基础数据。
* 包含：
  * 评论数据与投诉数据的整合（按订单、产品、门店、时间维度关联）。
  * 评价/投诉字段的统一整理与日期处理。
  * 导出汇总 parquet，供后续主题建模、好评率/投诉率分析与问题聚类使用。
* 具体对应的文件：`9-1-review-overview.ipynb.py`
* 输出：评论汇总 parquet、投诉汇总 parquet、后续分析所需中间数据。
